  ...
  return (
    <><Navbar />
      <div style={{maxWidth:'700px',margin:'2rem auto',padding:'0 2rem'}}>
        <h1 style={{fontSize:'1.5rem',color:'var(--text-heading)'}}>Manage Quiz</h1>

        {/* Quiz Settings */}
        {quiz && (
          <div style={{background:'var(--bg-card)',padding:'1.25rem',borderRadius:'12px',border:'1px solid var(--border)',boxShadow:'var(--shadow-sm)',marginBottom:'1.5rem'}}>
            <h2 style={{fontSize:'1rem',color:'var(--text-heading)',margin:'0 0 1rem'}}>Quiz Settings</h2>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
              <div>
                <label style={{display:'block',fontSize:'0.8rem',fontWeight:600,color:'var(--text-secondary)',marginBottom:'0.25rem'}}>Pass Mark (%)</label>
                <input
              ...
              <div>
                <label style={{display:'block',fontSize:'0.8rem',fontWeight:600,color:'var(--text-secondary)',marginBottom:'0.25rem'}}>Time Limit (minutes)</label>
                <input
                  type="number" min="0" style={inputStyle}
                  value={quizSettings.timeLimit}
                  onChange={e => setQuizSettings({...quizSettings, timeLimit: parseInt(e.target.value) || 0})}
                />
                <span style={{fontSize:'0.72rem',color:'var(--text-muted)'}}>0 = unlimited</span>
              </div>
            </div>
            <button onClick={saveQuizSettings} disabled={savingSettings} style={{marginTop:'0.75rem',background:'var(--accent)',color:'#fff',border:'none',padding:'0.5rem 1.25rem',borderRadius:'8px',fontWeight:600,fontSize:'0.8rem',cursor:'pointer'}}>
              {savingSettings ? 'Saving...' : 'Save Settings'}
            </button>
          </div>
        )}

        <h2 style={{fontSize:'1.1rem',color:'var(--text-heading)',margin:'1.5rem 0 1rem'}}>Questions ({questions.length})</h2>
        {questions.length === 0 && <p style={{color:'var(--text-muted)',fontSize:'0.85rem'}}>No questions yet. Add one below.</p>}
        {questions.map((q, i) => (
          <div key={q._id} style={{background:'var(--bg-card)',padding:'1rem',borderRadius:'10px',marginBottom:'0.75rem',border:'1px solid var(--border)',boxShadow:'var(--shadow-sm)'}}>
            {editingQ && editingQ._id === q._id ? (
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'0.5rem'}}>
                ...
                {editingQ.options.map((opt, j) => (
                  <div key={j} style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
                    ...
                    {editingQ.correctIndex === j && <span style={{fontSize:'0.75rem',fontWeight:700,color:'var(--success)',whiteSpace:'nowrap'}}>✓ Correct</span>}
                  </div>
                ))}
                ...
                <div style={{display:'flex',gap:'0.5rem'}}>
                  ...
                  <button onClick={() => setEditingQ(null)} style={{background:'var(--bg-muted)',color:'var(--text-secondary)',border:'1px solid var(--border)',padding:'0.45rem 1rem',borderRadius:'6px',fontWeight:600,fontSize:'0.8rem',cursor:'pointer'}}>Cancel</button>
                </div>
              </div>
            ) : (
              /* View mode */
              <>
                <div style={{display:'flex',justifyContent:'space-between'}}>
                  <strong style={{fontSize:'0.9rem',color:'var(--text-heading)'}}>Q{i+1}: {q.text}</strong>
                  <div style={{display:'flex',gap:'0.75rem'}}>
                    <button onClick={() => startEdit(q)} style={{color:'var(--accent)',background:'none',border:'none',cursor:'pointer',fontSize:'0.8rem',fontWeight:600}}>Edit</button>
                    <button onClick={() => deleteQuestion(q._id)} style={{color:'var(--danger)',background:'none',border:'none',cursor:'pointer',fontSize:'0.8rem',fontWeight:600}}>Delete</button>
                  </div>
                </div>
                <div style={{fontSize:'0.8rem',color:'var(--text-secondary)',marginTop:'0.4rem',display:'flex',flexDirection:'column',gap:'0.2rem'}}>
                  {q.options.map((o, j) => (
                    <div key={j} style={{
                      display:'flex', alignItems:'center', gap:'0.4rem',
                      padding:'0.25rem 0.5rem', borderRadius:'6px',
                      background: j === q.correctIndex ? 'var(--success-bg)' : 'transparent',
                      color: j === q.correctIndex ? 'var(--success)' : 'var(--text-secondary)',
                ...
                {q.explanation && <p style={{fontSize:'0.75rem',color:'var(--text-muted)',marginTop:'0.35rem',fontStyle:'italic'}}>💡 {q.explanation}</p>}
              </>
            )}
          </div>
        ))}

        <h2 style={{fontSize:'1.1rem',color:'var(--text-heading)',margin:'2rem 0 1rem'}}>Add New Question</h2>
        <div style={{background:'var(--bg-card)',padding:'1.25rem',borderRadius:'12px',border:'1px solid var(--border)',boxShadow:'var(--shadow-sm)',display:'flex',flexDirection:'column',gap:'0.75rem'}}>
          ...
          <label style={{fontSize:'0.78rem',fontWeight:600,color:'var(--text-secondary)'}}>Select the correct answer:</label>
          {newQ.options.map((opt, i) => (
            <div key={i} style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>
              ...
              {newQ.correctIndex === i && <span style={{fontSize:'0.75rem',fontWeight:700,color:'var(--success)',whiteSpace:'nowrap'}}>✓ Correct</span>}
            </div>
          ))}
          ...
          <button onClick={addQuestion} style={{background:'var(--accent)',color:'#fff',border:'none',padding:'0.7rem',borderRadius:'8px',fontWeight:600,cursor:'pointer'}}>Add Question</button>
        </div>
      </div>
    </>
  );
